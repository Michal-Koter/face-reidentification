...
class GoogleFace(pl.LightningModule):
    """
    A PyTorch Lightning Module for a GoogleNet-based face verification model.

    This model uses a pretrained GoogleNet backbone for feature extraction, followed by a fully connected layer
    for classification. The model is designed for binary classification tasks such as face verification.

    Attributes:
        backbone (nn.Sequential): The GoogleNet backbone excluding the final classification layer.
    ...
    def __init__(self, num_classes: int = 2) -> None:
        """
        Initializes the GoogleFace model with the specified number of output classes.
        ...
        self.backbone = nn.Sequential(*list(googlenet.children())[:-1])
