...
class DenseFace(pl.LightningModule):
    """
    A PyTorch Lightning Module for a DenseNet-based face verification model.

    This model uses a pretrained DenseNet-121 backbone for feature extraction, followed by a fully connected layer
    for classification. The model is designed for binary classification tasks such as face verification.

    Attributes:
        backbone (nn.Sequential): The DenseNet-121 backbone excluding the final classification layer.
    ...
    def __init__(self, num_classes: int = 2) -> None:
        """
        Initializes the DenseFace model with the specified number of output classes.
